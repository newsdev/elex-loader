...
function get_national_results {
    curl --compressed -o /tmp/results_national_$RACEDATE.json "http://int-elex-stg-east.newsdev.net/elections/$RACEDATE?apiKey=$AP_NAT_KEY&format=json&level=ru&national=true"
}

function get_local_results {
    curl --compressed -o /tmp/results_local_$RACEDATE.json "http://int-elex-stg-east.newsdev.net/elections/$RACEDATE?apiKey=$AP_LOC_KEY&format=json&level=ru&national=false"
}
